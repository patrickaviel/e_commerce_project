...
<head>
    ...
    <title>E-Shoepify - Profile</title>

    <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-eOJMYsd53ii+scO/bJGFsiCZc+5NDVN2yr8+0RDqr0Ql0h+rP48ckxlpbzKgwra6" crossorigin="anonymous">
    <!-- Personal CSS -->
    <link rel="stylesheet" href="<?php base_url() ?>/assets/stylesheets/user_profile.css">
    <!-- fontawesome cdn -->
        ...
        <div class="position-relative overflow-hidden bg-light banner-bgs">
            <form action="/users/save_profile" method="POST" class="row g-3 hero-style w-50 mx-auto p-4 m-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" id="contact_no" name="contact_no" value="<?= $this->session->userdata('user_contact_no')?>" placeholder="Ex: 09123456789">
                    <?php echo form_error('contact_no'); ?>
                </div>
                <div class="col-md-6">
                    <label for="first_name" class="form-label">First Name</label>
                    <input type="text" class="form-control" id="first_name" name="first_name" value="<?= $this->session->userdata('user_first_name')?>" placeholder="Juan">
                    <?php echo form_error('first_name'); ?>
                </div>
                <div class="col-md-6">
                    <label for="last_name" class="form-label">Last Name</label>
                    <input type="text" class="form-control" id="last_name" name="last_name" value="<?= $this->session->userdata('user_last_name')?>" placeholder="Dela Cruz">
                ...
                <div class="col-2">
                    ...
                    <input type="text" class="form-control" id="house_no" name="house_no" value="<?= $this->session->userdata('house_no')?>" placeholder="1234">
                    <?php echo form_error('house_no'); ?>
                </div>
                <div class="col-5">
                    <label for="barangay" class="form-label">Barangay</label>
                    <input type="text" class="form-control" id="barangay" name="barangay" value="<?= $this->session->userdata('barangay')?>" placeholder="Main St">
                    <?php echo form_error('barangay'); ?>
                </div>
                <div class="col-md-5">
                    <label for="municipality" class="form-label">Municipality</label>
                    <input type="text" class="form-control" id="municipality" name="municipality" value="<?= $this->session->userdata('municipality')?>" placeholder="Ex: San Fabian">
                    <?php echo form_error('municipality'); ?>
                </div>
                <div class="col-md-10">
                    <label for="province" class="form-label">Province</label>
                    <input type="text" class="form-control" id="province" name="province" value="<?= $this->session->userdata('province')?>" placeholder="Ex: Pangasinan">
                    <?php echo form_error('province'); ?>
                </div>
                <div class="col-md-2">
                    <label for="zip" class="form-label">Zip Code</label>
                    <input type="text" class="form-control" id="zip" name="zip" value="<?= $this->session->userdata('zip_code')?>" placeholder="Code">
                    <?php echo form_error('zip'); ?>
                </div>
                <div class="col-12">
                <input type="submit" class="btn btn-warning mt-3" value="Save Profile">
                </div>
            </form>
